namespace HRDepartment.Client.Api;

public class HRDepartmentApiWrapper(IConfiguration configuration): IHRDepartmentApiWrapper
{
    ...
    public async Task CreatePosition(PositionPostDto positionPostDto) => await client.PositionPOSTAsync(positionPostDto);

    public async Task<ICollection<PositionGetDto>> GetAllPositions() => await client.PositionAllAsync();

    public async Task<PositionGetDto> GetPositionById(int id) => await client.PositionGETAsync(id);

    public async Task UpdatePosition(int id, PositionPostDto positionPostDto) => await client.PositionPUTAsync(id, positionPostDto);

    public async Task DeletePosition(int id) => await client.PositionDELETEAsync(id);
}